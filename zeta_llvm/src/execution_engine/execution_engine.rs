    ...
    pub fn new(mut module: core::Module) -> Result<(ExecutionEngine, FrozenModule), String> {
        ...
        unsafe {
            ...
            if LLVMCreateExecutionEngineForModule(&mut ee, module.to_ref(), &mut error) > 0 {
                ...
                Ok((ExecutionEngine {ee, _memory_manager: None}, FrozenModule {module}))
            }
        }
    }
        ...
        unsafe {
            ...
            if LLVMCreateMCJITCompilerForModule(&mut ee,
                ...
                match self.memory_manager {
                    Some(memory_manager) => {
                        Ok((ExecutionEngine {ee, _memory_manager: Some(memory_manager)}, FrozenModule {module}))
                    }
                    None => Ok((ExecutionEngine {ee, _memory_manager: None}, FrozenModule {module}))
                }
            }
        }
    }
