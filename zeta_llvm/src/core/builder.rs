...
impl Builder {
    ...
    pub fn build_cond_br(&mut self, cond: LLVMValueRef, then_bl: &dyn BasicBlock, else_bl: &dyn BasicBlock) -> LLVMValueRef {
        unsafe {
            LLVMBuildCondBr(self.to_ref(), cond, then_bl.to_ref(), else_bl.to_ref())
        }
    }

    pub fn build_switch(&mut self, v: LLVMValueRef, else_bl: &dyn BasicBlock, num_cases: u32) -> LLVMValueRef {
        unsafe {
    ...
    pub fn build_invoke(&mut self, func: LLVMValueRef, args: &mut [LLVMValueRef], then_bl: &dyn BasicBlock, catch_bl: &dyn BasicBlock, name: &str) -> LLVMValueRef {
        let name = CString::new(name).unwrap();
        unsafe {
    ...
    pub fn build_landing_pad(&mut self, func: LLVMValueRef, args: &mut [LLVMValueRef], then_bl: &dyn BasicBlock, catch_bl: &dyn BasicBlock, name: &str) -> LLVMValueRef {
        let name = CString::new(name).unwrap();
        unsafe {
            LLVMBuildInvoke(self.to_ref(),
                            func,
    ...
    pub fn build_select(&mut self, cond: LLVMValueRef, then_bl: &dyn BasicBlock, else_bl: &dyn BasicBlock, name: &str) -> LLVMValueRef {
        let name = CString::new(name).unwrap();
        unsafe {
