...
impl SectionMemoryManager {
    pub fn new() -> Self {
        let memory_manager = unsafe {
            wrappers::LLVM_CreateSectionMemoryManager()
        };
...
impl ParentBSMM {
    pub fn allocate_code_section(&mut self,
        ...
        unsafe {
            wrappers::LLVM_BSMMCallParentAllocateCodeSection(self.0,
                                                             size, alignment, section_id,
    ...
    pub fn allocate_date_section(&mut self,
                                 ...
                                 section_id: c_uint,
                                 section_name: &str,
                                 is_read_only: bool) -> *mut u8 {
        let name = std::ffi::CString::new(section_name).unwrap();
        unsafe {
            wrappers::LLVM_BSMMCallParentAllocateDataSection(self.0,
                                                             size, alignment, section_id,
                                                             name.as_ptr() as *const c_char,
    ...
    pub fn invalidate_instruction_cache(&mut self) {
        unsafe {
            wrappers::LLVM_BSMMCallParentInvalidateInstructionCache(self.0)
        }
    }
...
pub struct BSMMFunctions<'a, 'b, 'c, 'd, 'e, 'f> {
    allocate_code_section: Option<Box<dyn FnMut(ParentBSMM,
                                            uintptr_t,
                                            u32,
                                            u32,
                                            &str) -> *mut u8 + 'a>>,

    allocate_data_section: Option<Box<dyn FnMut(ParentBSMM,
                                            uintptr_t,
                                            u32,
                                            u32,
                                            &str,
...
impl<'a, 'b, 'c, 'd, 'e, 'f> BindingSectionMemoryManagerBuilder<'a, 'b, 'c, 'd, 'e, 'f> {
    ...
    pub fn set_finalize_memory<F>(mut self, finalize_memory: F) -> Self
        where F: 'c + FnMut(ParentBSMM) -> Result<(), String> {
    ...
    pub fn set_invalidate_instruction_cache<F>(mut self, invalidate_instruction_cache: F) -> Self
        where F: 'd + FnMut(ParentBSMM) {
            self.bsmm_functions.invalidate_instruction_cache = Some(Box::new(invalidate_instruction_cache));
            self
    }

    pub fn set_get_symbol_address<F>(mut self, get_symbol_address: F) -> Self
        where F: 'e + FnMut(ParentBSMM, &str) -> u64 {
    ...
    pub fn create(mut self) -> BindingSectionMemoryManager<'a, 'b, 'c, 'd, 'e, 'f> {
        ...
        extern fn allocate_code_section_callback(opaque: *mut c_void,
            ...
            unsafe {
                ...
                let cstr_buf = std::ffi::CStr::from_ptr(section_name);
                let section_name = std::str::from_utf8_unchecked(cstr_buf.to_bytes());
                bsmm_functions.allocate_code_section.iter_mut().next().unwrap()
                    (parent, size, alignment, section_id, section_name)
            }
        }
        ...
        extern fn allocate_data_section_callback(opaque: *mut c_void,
                                                 ...
                                                 is_read_only: LLVMBool) -> *mut u8 {
            let parent = ParentBSMM(mm);
            unsafe {
                let bsmm_functions = &mut *(opaque as *mut BSMMFunctions);
                let cstr_buf = std::ffi::CStr::from_ptr(section_name);
                let section_name = std::str::from_utf8_unchecked(cstr_buf.to_bytes());
                bsmm_functions.allocate_data_section.iter_mut().next().unwrap()
                    (parent, size, alignment, section_id, section_name, is_read_only > 0)
            }
        }

        extern fn finalize_memory_callback(opaque: *mut c_void,
                                           mm: LLVMMCJITMemoryManagerRef,
                                           error_msg: *mut *mut c_char) -> LLVMBool {
            let parent = ParentBSMM(mm);
            unsafe {
                let bsmm_functions = &mut *(opaque as *mut BSMMFunctions);
        ...
        extern fn invalidate_instruction_cache_callback(opaque: *mut c_void,
                                                        mm: LLVMMCJITMemoryManagerRef) {
            let parent = ParentBSMM(mm);
            unsafe {
                let bsmm_functions = &mut *(opaque as *mut BSMMFunctions);
                bsmm_functions.invalidate_instruction_cache.iter_mut().next().unwrap()(parent)
            }
        }

        extern fn get_symbol_address_callback(opaque: *mut c_void,
                                              mm: LLVMMCJITMemoryManagerRef,
        ...
        extern fn destroy_callback(opaque: *mut c_void) {
            unsafe {
                let bsmm_functions = &mut *(opaque as *mut BSMMFunctions);
        ...
        let allocate_code_section = unsafe {
            match self.bsmm_functions.allocate_code_section {
                Some(_) => allocate_code_section_callback
                    as wrappers::LLVM_BSMMAllocateCodeSectionCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMAllocateCodeSectionCallback>(std::ptr::null_mut::<c_void>())
            }
        };

        let allocate_data_section = unsafe {
            match self.bsmm_functions.allocate_data_section {
                Some(_) => allocate_data_section_callback
                    as wrappers::LLVM_BSMMAllocateDataSectionCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMAllocateDataSectionCallback>(std::ptr::null_mut::<c_void>())
            }
        };

        let finalize_memory = unsafe {
            match self.bsmm_functions.finalize_memory {
                Some(_) => finalize_memory_callback
                    as wrappers::LLVM_BSMMFinalizeMemoryCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMFinalizeMemoryCallback>(std::ptr::null_mut::<c_void>())
            }
        };

        let invalidate_instruction_cache =  unsafe {
            match self.bsmm_functions.invalidate_instruction_cache {
                Some(_) => invalidate_instruction_cache_callback
                    as wrappers::LLVM_BSMMInvalidateInstructionCacheCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMInvalidateInstructionCacheCallback>(std::ptr::null_mut::<c_void>())
            }
        };

        let get_symbol_address =  unsafe {
            match self.bsmm_functions.get_symbol_address {
                Some(_) => get_symbol_address_callback
                    as wrappers::LLVM_BSMMGetSymbolAddressCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMGetSymbolAddressCallback>(std::ptr::null_mut::<c_void>())
            }
        };

        let destroy =  unsafe {
            match self.bsmm_functions.destroy {
                Some(_) => destroy_callback
                    as wrappers::LLVM_BSMMDestroyCallback,
                None => std::mem::transmute::<*mut c_void,
                wrappers::LLVM_BSMMDestroyCallback>(std::ptr::null_mut::<c_void>())
            }
        };
