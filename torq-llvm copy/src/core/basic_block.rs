...
pub trait BasicBlock : LLVMRef<LLVMBasicBlockRef> + Value {
    fn get_parent(&self) -> FunctionRef {
        unsafe {
            FunctionRef::from_ref(LLVMGetBasicBlockParent(self.to_ref()))
        }
    }
    ...
    fn remove_from_parent(&mut self) {
        unsafe {
            LLVMRemoveBasicBlockFromParent(self.to_ref())
        }
    }

    fn move_before(&self, move_pos: &dyn BasicBlock) {
        unsafe {
            LLVMMoveBasicBlockBefore(self.to_ref(), move_pos.to_ref())
        }
    }

    fn move_after(&self, move_pos: &dyn BasicBlock) {
        unsafe {
            LLVMMoveBasicBlockAfter(self.to_ref(), move_pos.to_ref())
        }
    }
